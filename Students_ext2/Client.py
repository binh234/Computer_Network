...
class Client:
	...
	def createWidgets(self):
		...
		# self.setup["command"] = self.describeSession
	...
	def sendRtspRequest(self, requestCode):
		...
		if requestCode == self.SETUP and self.state == self.INIT:
			...
			# request = ...
			request = "SETUP {0} RTSP/1.0\nCSeg: {1}\nTransport: RTP/UDP; client_port= {2}".format(self.fileName, self.rtspSeq, self.rtpPort)
		...
		# Play request
			...
			# Write the RTSP request to be sent.
			# request = ...
			request = "PLAY {0} RTSP/1.0\nCSeg: {1}\nSession: {2}".format(self.fileName, self.rtspSeq, self.sessionId)
